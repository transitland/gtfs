...
describe Fetch do
  ...
  context '.request' do
    it 'returns a response' do
      VCR.use_cassette('fetch') do
        response = {}
        Fetch.request(url) { |resp| response = JSON.parse(resp.read_body) }
    ...
    it 'follows a redirect' do
      VCR.use_cassette('fetch_redirect') do
        response = {}
        Fetch.request(url_redirect) { |resp| response = JSON.parse(resp.read_body) }
        response['url'].should eq(url)
      end
    end
    ...
    it 'follows SSL' do
      VCR.use_cassette('fetch_ssl') do
        response = {}
        Fetch.request(url_ssl) { |resp| response = JSON.parse(resp.read_body) }
    ...
    it 'follows a redirect no more than limit times' do
      VCR.use_cassette('fetch_redirect_fail') do
        expect {
          Fetch.request(url_redirect_many, limit:2) { |resp| response = JSON.parse(resp.read_body) }
    ...
    it 'raises errors' do
      VCR.use_cassette('fetch_fetch_404') do
        expect {
          Fetch.request(url_404, limit:2) { |resp| response = JSON.parse(resp.read_body) }
  ...
  context '.download' do
    it 'downloads to temp file' do
      VCR.use_cassette('fetch') do
        data = {}
        Fetch.download_to_tempfile(url) { |filename| data = JSON.parse(File.read(filename))}
    ...
    it 'removes tempfile' do
      VCR.use_cassette('fetch') do
        path = nil
        Fetch.download_to_tempfile(url) { |filename| path = filename }
    ...
    it 'downloads binary data' do
      VCR.use_cassette('fetch_binary') do
        data = nil
        Fetch.download_to_tempfile(url_binary) { |filename| data = File.read(filename) }
    ...
    it 'allows files smaller than maximum size' do
      VCR.use_cassette('fetch_binary') do
        data = nil
        Fetch.download_to_tempfile(url_binary, maxsize:2048) { |filename| data = File.read(filename) }
        Digest::MD5.new.update(data).hexdigest.should eq('355c7ebd00db307b91ecd23a4215174a')
      end
    end
    ...
    it 'raises error if response larger than maximum size' do
      VCR.use_cassette('fetch_binary') do
        expect {
          Fetch.download_to_tempfile(url_binary, maxsize:128) { |filename| }
